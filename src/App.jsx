...
function Shell({ role, user, onLogout, onUserUpdate, active, setActive, children }) {
  ...
  const notificationDestination = (item) => item.type.startsWith('booking') ? 'appointments' : item.type.startsWith('followup') ? 'followup' : item.type.startsWith('support') ? 'consult' : '';
  async function openNotice(item) { await readNotice(item); const destination = notificationDestination(item); if (destination && role === 'patient') { setActive(destination); setNoticeOpen(false); } }
  async function readAllNotices() { await api.readAllNotifications(); setNotices((current) => ({ notifications: current.notifications.map((item) => ({ ...item, read: true })), unread: 0 })); }
    ...
    <div className="main-frame">
      <header className="topbar">
        ...
        <div className="top-actions">
          ...
          <div className="notification-wrap"><button className="icon-button notification" aria-label="站内通知" onClick={() => setNoticeOpen(!noticeOpen)}><Bell size={19} />{notices.unread > 0 && <i />}</button>{noticeOpen && <div className="notification-panel"><div><strong>站内通知</strong><span>{notices.unread} 条未读</span>{notices.unread > 0 && <button className="notice-read-all" onClick={readAllNotices}>全部已读</button>}</div>{notices.notifications.length ? notices.notifications.slice(0,8).map((item)=><button className={item.read?'read':''} key={item.id} onClick={()=>openNotice(item)}><i/><div><strong>{item.title}</strong><p>{item.content}</p><time>{new Date(item.createdAt).toLocaleString('zh-CN')}{notificationDestination(item) && role === 'patient' ? ' · 查看详情' : ''}</time></div><ChevronRight size={15}/></button>) : <p className="empty-notice">暂无通知</p>}</div>}</div>
...
function LiveAppointments({ setActive }) {
  ...
  function exportCalendar(booking){const start=new Date(booking.appointmentAt);const end=new Date(start.getTime()+30*60000);const stamp=(date)=>date.toISOString().replace(/[-:]/g,'').replace(/\.\d{3}/,'');const safe=(value)=>String(value||'').replace(/([,;\\])/g,'\\$1').replace(/\n/g,'\\n');const ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//VertiBalance//Patient Booking//ZH-CN','CALSCALE:GREGORIAN','BEGIN:VEVENT',`UID:${booking.id}@vertibalance.local`,`DTSTAMP:${stamp(new Date())}`,`DTSTART:${stamp(start)}`,`DTEND:${stamp(end)}`,`SUMMARY:${safe(`眩衡就诊：${booking.department}`)}`,`DESCRIPTION:${safe(`${booking.doctor.name} ${booking.doctor.title}，挂号号 ${booking.id.slice(-6).toUpperCase()}`)}`,`LOCATION:${safe(booking.campus)}`,'END:VEVENT','END:VCALENDAR'].join('\r\n');const blob=new Blob([ics],{type:'text/calendar;charset=utf-8'});const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download=`VertiBalance-${booking.id.slice(-6)}.ics`;anchor.click();URL.revokeObjectURL(url)}
...
function LiveFollowups({ setActive }) {
  const [items, setItems] = useState([]); const [activeId, setActiveId] = useState(''); const [filter, setFilter] = useState('pending'); const [form, setForm] = useState({ severity: 3, frequency: 0, text: '', medicationTaken: true }); const [message, setMessage] = useState('');
  const load = () => api.followups().then((result) => setItems(result.followups));
  useEffect(() => { load().catch((error) => setMessage(error.message)); }, []);
  async function submit(id) { try { const result = await api.submitFollowup(id, form); setMessage(result.message); setActiveId(''); localStorage.removeItem(`vertibalance_followup_draft_${id}`); await load(); } catch (error) { setMessage(error.message); } }
  function openTask(id) { setActiveId(id); const saved = localStorage.getItem(`vertibalance_followup_draft_${id}`); setForm(saved ? JSON.parse(saved) : { severity: 3, frequency: 0, text: '', medicationTaken: true }); }
  useEffect(() => { if (activeId) localStorage.setItem(`vertibalance_followup_draft_${activeId}`, JSON.stringify(form)); }, [activeId, form]);
  const visible = items.filter((item) => filter === 'all' || (filter === 'pending' && item.status === 'pending') || (filter === 'completed' && item.status === 'completed') || (filter === 'abnormal' && item.abnormal));
  return <div className="page"><div className="page-title"><div><span className="eyebrow"><Activity size={15}/>诊后健康管理</span><h1>康复随访</h1><p>反馈真实症状变化；异常结果会标记给医生关注。</p></div></div>{message && <div className={`inline-feedback ${message.includes('异常') || message.includes('就医') ? 'warning' : 'success'}`}>{message}</div>}<div className="followup-filters">{[['pending','待完成'],['completed','已完成'],['abnormal','异常关注'],['all','全部']].map(([key,label])=><button className={filter===key?'active':''} key={key} onClick={()=>setFilter(key)}>{label}<b>{key==='all'?items.length:items.filter((item)=>key==='abnormal'?item.abnormal:item.status===key).length}</b></button>)}</div>{visible.length ? <div className="followup-grid">{visible.map((item,index) => { const overdue = item.status === 'pending' && new Date(item.dueAt).getTime() < Date.now(); return <div className={`followup-card live ${item.abnormal ? 'abnormal' : ''}`} key={item.id}><div className={`task-icon t${index%3}`}><ClipboardCheck size={20}/></div><div><span>{item.type} · 截止 {new Date(item.dueAt).toLocaleString('zh-CN')}</span><h3>{item.title}</h3><p>{item.status === 'completed' ? `已提交 · 严重程度 ${item.feedback?.severity}/10 · 过去 24 小时 ${item.feedback?.frequency ?? 0} 次` : '请按计划记录当前症状、发作频率和用药情况'}</p></div><em className={overdue?'overdue':''}>{item.abnormal ? '异常关注' : item.status === 'completed' ? '已完成' : overdue ? '已逾期' : '待完成'}</em>{item.status === 'pending' && <button className="primary-button" onClick={() => openTask(item.id)}>{activeId === item.id ? '收起填写' : '开始填写'}</button>}{item.abnormal && item.status === 'completed' && <button className="outline-button" onClick={() => setActive('reports')}>查看问诊记录</button>}{activeId === item.id && <div className="followup-form"><label>当前眩晕严重程度 <strong>{form.severity}/10</strong><input type="range" min="0" max="10" value={form.severity} onChange={(event) => setForm({...form,severity:Number(event.target.value)})}/></label><label>过去 24 小时发作次数 <input type="number" min="0" max="1000" value={form.frequency} onChange={(event) => setForm({...form,frequency:Number(event.target.value)})}/></label><label>症状变化<textarea value={form.text} onChange={(event)=>setForm({...form,text:event.target.value})} placeholder="例如：今天眩晕次数减少，但起床时仍明显…"/></label><label className="check-line"><input type="checkbox" checked={form.medicationTaken} onChange={(event)=>setForm({...form,medicationTaken:event.target.checked})}/>已按医嘱用药</label><button className="dark-button" onClick={() => submit(item.id)}>提交随访反馈</button></div>}</div>})}</div> : <EmptyState icon={ClipboardCheck} title="暂无匹配随访" message="当前筛选下没有任务，切换到全部即可查看历史记录。"/>}</div>;
}
...
function PatientDocuments() {
  const [items,setItems]=useState([]);const [consultations,setConsultations]=useState([]);const [file,setFile]=useState(null);const [category,setCategory]=useState('检查资料');const [consultationId,setConsultationId]=useState('');const [message,setMessage]=useState('');const [uploading,setUploading]=useState(false);const [preview,setPreviewState]=useState(null);
  const load=()=>Promise.all([api.uploads(),api.consultations()]).then(([uploads,records])=>{setItems(uploads.uploads);setConsultations(records.consultations)});useEffect(()=>{load().catch((error)=>setMessage(error.message));},[]);
  async function upload(){if(!file){setMessage('请先选择 PDF、JPG 或 PNG 文件');return}setUploading(true);const body=new FormData();body.append('file',file);body.append('category',category);if(consultationId)body.append('consultationId',consultationId);try{await api.upload(body);setMessage('资料已加密保存，挂号移交后对应医生可查看。');setFile(null);await load()}catch(error){setMessage(error.message)}finally{setUploading(false)}}
  async function remove(item){if(!window.confirm(`确认删除“${item.name}”？删除后无法恢复。`))return;try{await api.deleteUpload(item.id);setMessage('资料已删除');await load()}catch(error){setMessage(error.message)}}
  async function setPreview(item){if(!item){if(preview?.url)URL.revokeObjectURL(preview.url);setPreviewState(null);return}try{const response=await fetch(`/api/v1/uploads/${item.id}/preview`,{headers:{Authorization:`Bearer ${getAuthToken()}`}});if(!response.ok)throw new Error('资料预览失败或当前无访问权限');const blob=await response.blob();setPreviewState({...item,url:URL.createObjectURL(blob)})}catch(error){setMessage(error.message)}}
  ...
  return <div className="page"><div className="page-title"><div><span className="eyebrow"><FileClock size={15}/>患者资料中心</span><h1>健康资料</h1><p>上传检查单或病史图片，挂号后安全移交给对应医生。</p></div></div>{message&&<div className={`inline-feedback ${message.includes('已加密')||message.includes('已删除')?'success':''}`}>{message}</div>}<div className="document-layout"><div className="panel upload-box"><div className="placeholder-icon"><FileText size={30}/></div><h2>上传病史资料</h2><p>仅支持 PDF、JPG、PNG，单个文件不超过 5MB；禁止上传可执行文件。</p><label className="file-picker"><input type="file" accept="application/pdf,image/jpeg,image/png" onChange={(e)=>setFile(e.target.files?.[0]||null)}/><span>{file?file.name:'选择文件'}</span></label><label>资料类型<select value={category} onChange={(e)=>setCategory(e.target.value)}><option>检查资料</option><option>既往病历</option><option>用药清单</option><option>其他资料</option></select></label><label>关联问诊（可选）<select value={consultationId} onChange={(e)=>setConsultationId(e.target.value)}><option value="">不关联具体问诊</option>{consultations.map((item)=><option key={item.id} value={item.id}>{new Date(item.createdAt).toLocaleString('zh-CN')} · {item.status==='in_progress'?'进行中':'已完成'}</option>)}</select></label><button className="primary-button" disabled={uploading} onClick={upload}>{uploading?'安全上传中…':'确认上传'}</button></div><div className="panel document-list"><div className="section-heading"><div><h2>我的资料</h2><p>{items.length} 个已加密文件</p></div></div>{items.length?items.map((item)=><div className="document-row" key={item.id}><div className="task-icon"><FileText size={19}/></div><div className="document-meta"><strong>{item.name}</strong><span>{item.category} · {(item.size/1024).toFixed(1)}KB · {new Date(item.createdAt).toLocaleString('zh-CN')}</span></div><div className="document-actions"><button className="soft-button" onClick={()=>setPreview(item)}>预览</button><button className="outline-button" onClick={()=>download(item)}>下载</button><button className="danger-button" onClick={()=>remove(item)}>删除</button></div></div>):<p className="empty-inline">暂无上传资料</p>}</div></div>{preview&&<div className="modal-backdrop" onClick={()=>setPreview(null)}><section className="knowledge-modal document-preview" role="dialog" aria-modal="true" aria-labelledby="preview-title" onClick={(event)=>event.stopPropagation()}><button className="modal-close" aria-label="关闭资料预览" onClick={()=>setPreview(null)}><X size={19}/></button><span className="eyebrow"><FileText size={15}/>资料预览</span><h2 id="preview-title">{preview.name}</h2>{preview.mimeType==='application/pdf'?<iframe title={preview.name} src={previewUrl(preview)}/>:<img src={previewUrl(preview)} alt={preview.name}/>}<p className="preview-note">资料仅在当前授权会话中展示，医生需在挂号移交后访问。</p></section></div>}</div>;
}
