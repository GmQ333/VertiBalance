...
const DISCLAIMER =
  "本系统仅用于辅助筛查和健康信息参考，不能替代医生面诊和临床诊断。";
const ROLE_LABELS = { patient: "患者端", doctor: "医生端", admin: "管理端" };
const patientNav = [
  ["overview", LayoutDashboard, "健康首页"],
  ["consult", MessageCircleMore, "智能问诊"],
  ["reports", FileText, "问诊记录"],
  ["appointments", CalendarDays, "我的挂号"],
  ["followup", Activity, "康复随访"],
  ["documents", FileClock, "健康资料"],
  ["education", BookOpen, "健康科普"],
];
const doctorNav = [
  ["workspace", LayoutDashboard, "接诊工作台"],
  ["patients", Users, "我的患者"],
  ["followups", ClipboardCheck, "随访任务"],
  ["schedule", CalendarDays, "我的排班"],
];
const adminNav = [
  ["dashboard", BarChart3, "运营总览"],
  ["users", UserRoundCog, "用户与角色"],
  ["schedules", CalendarDays, "医生与排班"],
  ["models", Brain, "模型管理"],
  ["knowledge", Database, "知识库"],
  ["audit", ShieldCheck, "审计与安全"],
];
...
function RoleDisplay({ role }) {
  return (
    <div
      ...
      aria-label={`当前工作空间：${ROLE_LABELS[role]}`}
    >
...
function ProfileMenu({ role, user, onLogout, onProfile, onSecurity }) {
  ...
  const subtitle =
    role === "patient"
      ? "患者"
      : role === "doctor"
        ? `${user.department || "临床科室"} · ${user.title || "医生"}`
        : "平台管理员";
  useEffect(() => {
...
function Shell({
  ...
  const notificationDestination = (item) =>
    item.type.startsWith("booking")
      ? "appointments"
      : item.type.startsWith("followup")
        ? "followup"
        : item.type.startsWith("support")
          ? "consult"
          : "";
  async function openNotice(item) {
    await readNotice(item);
    const destination = notificationDestination(item);
    if (destination && role === "patient") {
  ...
  async function submitFeedback() {
    try {
      ...
      setFeedbackMessage("感谢反馈，我们已记录你的意见。");
    ...
    <div className={`app-shell role-${role}`}>
      <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
        ...
        <div className="sidebar-foot">
          ...
          <button className="help-link" onClick={() => setFeedbackOpen(true)}>
            <HelpCircle size={18} />
            帮助与反馈
          </button>
        </div>
              ...
              {noticeOpen && (
                <div className="notification-panel">
                  ...
                  {notices.notifications.length ? (
                    notices.notifications.slice(0, 8).map((item) => (
                      <button
                      ...
                      >
                        ...
                        <div>
                          ...
                          <time>
                            ...
                            {notificationDestination(item) && role === "patient"
                              ? " · 查看详情"
                              : ""}
                          </time>
                        </div>
                    ...
                    <p className="empty-notice">暂无通知</p>
                  )}
                </div>
              )}
            </div>
      ...
      {feedbackOpen && (
        <div className="modal-backdrop" onClick={() => setFeedbackOpen(false)}>
          <div
            className="knowledge-modal feedback-modal"
          ...
          >
            ...
            <label>
              ...
              <textarea
                ...
                placeholder="请勿在反馈中填写完整病历或敏感个人信息…"
              />
            </label>
            {feedbackMessage && (
              <div className="inline-feedback success">{feedbackMessage}</div>
            )}
            <button className="primary-button full" onClick={submitFeedback}>
              提交反馈
            </button>
          </div>
        </div>
          ...
          >
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) =>
                  setProfileForm({ ...profileForm, age: event.target.value })
                }
              />
            </label>
            {profileMessage && (
              <div
                className={`inline-feedback ${profileMessage.includes("更新") ? "success" : ""}`}
              >
                {profileMessage}
              </div>
            )}
...
function PatientOverviewInsights({ data, setActive }) {
  ...
  reports.forEach((item) => {
    if (counts[item.riskLevel] !== undefined) counts[item.riskLevel] += 1;
  });
  const label = { emergency: "紧急", high: "高", medium: "中", low: "低" };
  return (
    <section className="overview-insights">
      <div className="panel insight-card risk-trend">
        <div className="section-heading">
          <div>
            <h2>近期风险概览</h2>
            <p>最近 {recent.length} 份结构化问诊报告</p>
          </div>
        ...
        <div className="risk-bars">
          {recent.length ? (
            recent.map((item) => (
              <div className="risk-bar-item" key={item.id}>
                <div
                  className={`risk-bar ${item.riskLevel}`}
                  style={{
                    height: `${Math.max(20, (["low", "medium", "high", "emergency"].indexOf(item.riskLevel) + 1) * 20)}%`,
                  }}
                >
                  <span>{label[item.riskLevel] || "待评估"}</span>
                </div>
                <small>
            ...
            <p className="empty-inline">完成问诊后，这里会显示风险变化</p>
          )}
        </div>
        <div className="risk-summary">
          <span className="emergency">紧急 {counts.emergency}</span>
          <span className="high">高风险 {counts.high}</span>
          <span className="medium">中风险 {counts.medium}</span>
          <span className="low">低风险 {counts.low}</span>
        </div>
        {recent[0] && (
          <p className="insight-note">
            最近一次评估：{label[recent[0].riskLevel] || "待评估"}风险 ·{" "}
            {recent[0].careTimeframe || "请查看报告中的就医时效"}
          </p>
        )}
      </div>
      <div className="panel insight-card care-summary">
        <div className="section-heading">
          <div>
            <h2>健康安排</h2>
            <p>近期任务与资料概览</p>
        ...
        <div className="care-stat">
          <strong>{data?.followups?.length || 0}</strong>
          <span>待完成随访</span>
          <button onClick={() => setActive("followup")}>
            去完成
            <ArrowRight size={14} />
          </button>
        </div>
        <div className="care-stat">
          <strong>{data?.upcomingBooking ? "1" : "0"}</strong>
          <span>近期预约</span>
          <button onClick={() => setActive("appointments")}>
            {data?.upcomingBooking ? "查看详情" : "预约医生"}
            <ArrowRight size={14} />
          </button>
        </div>
        <div className="care-stat">
          <strong>{data?.reports?.length || 0}</strong>
          <span>结构化报告</span>
...
function LivePatientOverview({ setActive, user }) {
  ...
  useEffect(() => {
    api.patientDashboard().then(setData);
  }, []);
  return (
    <div className="page patient-home">
      <section className="patient-hero">
        <div className="hero-copy">
          ...
          <h1>你好，{user.name}</h1>
          <p>
            如果出现眩晕、头昏或失衡，我会先用规则筛查危险信号，再由专业模型进行引导式追问。
          </p>
      ...
      <PatientOverviewInsights data={data} setActive={setActive} />
      <div className="quick-grid">
        <button
          className="quick-card indigo"
          onClick={() => setActive("consult")}
        >
          <span>
        ...
        >
          ...
          <div>
            <strong>
              {data?.upcomingBooking ? "已有预约" : "预约专科医生"}
            </strong>
            <p>
              {data?.upcomingBooking
                ? new Date(data.upcomingBooking.appointmentAt).toLocaleString(
                    "zh-CN",
                  )
                : "查看推荐医生与实时号源"}
            </p>
            ...
            {data?.followups.length ? (
              data.followups.slice(0, 3).map((item, index) => (
                <div className="timeline-row" key={item.id}>
                  ...
                  <em>{item.abnormal ? "异常关注" : "待完成"}</em>
                </div>
              ))
            ) : (
              <p className="empty-inline">暂无待办随访任务</p>
            )}
          </div>
        </section>
        <section className="panel knowledge-preview">
          ...
          {data?.knowledge[0] && (
            <div className="article-feature">
              ...
              <div>
                <span className="tag">{data.knowledge[0].category}</span>
                <h3>{data.knowledge[0].title}</h3>
                <p>{data.knowledge[0].summary}</p>
              </div>
            </div>
          )}
        </section>
...
function LivePatientReport({ setActive, latestReport }) {
  ...
  useEffect(() => {
    api
      ...
      .then((result) => {
        ...
        setSelectedId(
          (current) => current || result.consultations[0]?.id || "",
        );
      })
    ...
    return (
      <div className="page records-page">
        ...
        <EmptyState
          ...
          title="暂无问诊记录"
          message={
            error || "开始一次智能问诊后，对话和结构化报告会保存在这里。"
          }
          action="开始智能问诊"
          onAction={() => setActive("consult")}
        />
      </div>
    );
  const consultation =
  ...
  const report =
    detail?.report ||
    (latestReport?.consultationId === selectedId ? latestReport : null);
  const statusLabels = {
    in_progress: "进行中",
  ...
  const visibleConsultations = consultations.filter(
    (item) =>
      ...
      (recordFilter === "report"
        ...
        : item.riskLevel === recordFilter),
  );
  const messages = detail?.messages || [];
  const directionItems = report?.possibleDirections?.length
    ? report.possibleDirections
    : ["疾病方向待医生进一步评估"];
  const riskMeta = report
    ? {
        emergency: {
          label: "紧急",
          timing: report.careTimeframe || "立即急诊或呼叫 120",
          tone: "emergency",
        },
        high: {
          label: "高",
          timing: report.careTimeframe || "24 小时内就医",
          tone: "high",
        },
        medium: {
          label: "中",
          timing: report.careTimeframe || "一周内就医",
          tone: "moderate",
        },
        low: {
          label: "低",
          timing: report.careTimeframe || "按需就医并留意变化",
          tone: "low",
        },
      }[report.riskLevel] || {
        label: "待评估",
  ...
  return (
    <div className="page records-page">
      <button className="back-button" onClick={() => setActive("overview")}>
        <ArrowLeft size={17} />
        返回健康首页
      </button>
      <div className="page-title">
        ...
        <button className="primary-button" onClick={() => setActive("consult")}>
          <Plus size={17} />
          开始新问诊
        </button>
      </div>
      ...
      <div className="report-filters">
        {[
          ...
          <button
            className={recordFilter === key ? "active" : ""}
            key={key}
            onClick={() => {
              setRecordFilter(key);
              const first = consultations.find(
                (item) =>
                  key === "all" ||
                  (key === "report"
                    ? item.status !== "in_progress"
      ...
      <div className="records-layout">
        <aside className="panel record-index">
          <div className="section-heading">
            <div>
              <h2>问诊记录</h2>
              <p>{visibleConsultations.length} 条匹配记录</p>
            </div>
          </div>
        ...
        <section className="record-content">
          {detailLoading ? (
            <div className="panel record-loading">正在读取问诊详情…</div>
          ) : (
            <>
              <div className="panel record-summary">
                <div>
                  <span className={`record-status ${consultation?.status}`}>
                    {statusLabel}
                  </span>
                  <h2>问诊编号 {consultation?.id.slice(-12).toUpperCase()}</h2>
                  <p>
              ...
              <div className="panel timeline-panel">
                <div className="section-heading">
                  <div>
                    <h2>完整问诊对话</h2>
                    <p>{messages.length} 条消息，按发生时间保存</p>
                  </div>
                </div>
                <div className="conversation-record patient-conversation">
                  {messages.map((item) => (
                    <div className={item.role} key={item.id}>
                      <span>{item.role === "user" ? "我" : "眩衡助手"}</span>
                      <p>{item.content}</p>
                      <time>
                        {new Date(item.createdAt).toLocaleString("zh-CN")}
                      </time>
              ...
              {report ? (
                <div className="record-report">
                  <div className="report-heading">
                    ...
                    <div className={`risk-seal ${riskMeta.tone}`}>
                      <span>风险等级</span>
                      <strong>{riskMeta.label}</strong>
                      <small>{riskMeta.timing}</small>
                    </div>
                  </div>
                  <div
                    className={`report-notice ${report.generationSource === "fallback" ? "fallback" : ""}`}
                  >
                    <ShieldCheck size={19} />
                    <div>
                      <strong>
                      ...
                      <p>
                        {report.generationSource === "fallback"
                          ? "报告依据危险信号规则和已记录信息生成，请由医生进一步评估。"
                          : "报告用于帮助你与医生更高效地沟通，具体诊断和治疗方案需由医生面诊后决定。"}
                      </p>
                    </div>
                  </div>
                  <div className="report-grid">
                    <section className="panel report-main">
                      <h2>症状摘要</h2>
                      <p className="summary-text">{report.chiefComplaint}</p>
                      <div className="fact-grid">
                        <div>
                          <span>发作特点</span>
                          <strong>{report.episodeFeatures}</strong>
                        </div>
                        <div>
                          <span>主要诱因</span>
                          <strong>{report.triggers}</strong>
                        </div>
                        <div>
                          <span>伴随症状</span>
                          <strong>{report.accompanyingSymptoms}</strong>
                        </div>
                        <div>
                          <span>危险信号</span>
                          <strong
                            className={
                              report.dangerSignals?.length
                                ? "danger-text"
                                : "safe-text"
                            }
                          >
                            ...
                            {report.dangerSignals?.join("、") || "暂未识别"}
                          </strong>
                        </div>
                        <div>
                          <span>既往史</span>
                        ...
                        <div>
                          <span>当前用药</span>
                          <strong>{report.medications || "未采集"}</strong>
                        </div>
                      </div>
                      <h2>可能涉及的方向</h2>
                      <div className="direction-card">
                        ...
                        <div>
                          <strong>{directionItems.join("；")}</strong>
                          <p>{report.aiRiskNote}</p>
                        </div>
                      </div>
                    </section>
                    <aside>
                      <div className="panel recommendation">
                        <span className="tag">就医建议</span>
                        <h3>
                          {report.riskLevel === "emergency"
                            ? "请立即前往急诊或呼叫 120"
                            : report.riskLevel === "high"
                              ? "建议 24 小时内就医评估"
                              : `建议预约${report.recommendedDepartment}`}
                        </h3>
                        <div>
                          <Hospital size={17} />
                          <p>
                            <span>推荐科室</span>
                            <strong>{report.recommendedDepartment}</strong>
                          </p>
                        </div>
                        <div>
                          <Clock3 size={17} />
...
function LiveAppointments({ setActive }) {
  ...
  const load = () =>
    Promise.all([api.bookings(), api.schedules(), api.reports()])
      .then(([bookings, schedules, reports]) =>
        setData({
  ...
  const availableReports = data.reports.filter(
    (item) => !bookedConsultations.has(item.consultationId),
  );
  const sortedSchedules = [...data.schedules].sort((a, b) => {
    ...
    const report = data.reports.find(
      (item) => item.consultationId === selectedConsultationId,
    );
    const reportDepartment = report?.recommendedDepartment || "";
    const aMatch =
      reportDepartment &&
      (a.department.includes(reportDepartment.split("/")[0]) ||
        reportDepartment.includes(a.department));
    const bMatch =
      reportDepartment &&
      (b.department.includes(reportDepartment.split("/")[0]) ||
        reportDepartment.includes(b.department));
    return (
      Number(bMatch) - Number(aMatch) || a.startAt.localeCompare(b.startAt)
    );
  });
  async function book(schedule) {
    ...
    setMessage("");
    try {
      await api.createBooking(report.consultationId, schedule.id);
      setMessage("挂号成功，问诊报告已安全移交给对应医生。");
      await load();
    } catch (error) {
      setMessage(error.message);
    } finally {
  ...
  async function cancel(booking) {
    if (!window.confirm("确认取消这次挂号并释放号源？")) return;
    try {
      await api.cancelBooking(booking.id);
      setMessage("挂号已取消，号源已释放。");
      await load();
    } catch (error) {
      setMessage(error.message);
    }
  ...
  function exportCalendar(booking) {
    ...
    const ics = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//VertiBalance//Patient Booking//ZH-CN",
      "CALSCALE:GREGORIAN",
      "BEGIN:VEVENT",
      `UID:${booking.id}@vertibalance.local`,
      `DTSTAMP:${stamp(new Date())}`,
      `DTSTART:${stamp(start)}`,
      `DTEND:${stamp(end)}`,
      `SUMMARY:${safe(`眩衡就诊：${booking.department}`)}`,
      `DESCRIPTION:${safe(`${booking.doctor.name} ${booking.doctor.title}，挂号号 ${booking.id.slice(-6).toUpperCase()}`)}`,
      `LOCATION:${safe(booking.campus)}`,
      "END:VEVENT",
      "END:VCALENDAR",
    ].join("\r\n");
    ...
    anchor.href = url;
    anchor.download = `VertiBalance-${booking.id.slice(-6)}.ics`;
    anchor.click();
    ...
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>我的挂号</h1>
          <p>查看已确认预约，或使用问诊报告预约推荐门诊。</p>
        </div>
        <button className="outline-button" onClick={() => setActive("reports")}>
          <FileText size={17} />
          查看报告
        </button>
      </div>
      {message && (
        <div
          className={`inline-feedback ${message.includes("成功") ? "success" : ""}`}
        >
          {message}
        </div>
      )}
      <div className="service-section">
        ...
        {data.bookings.length ? (
            ...
            {data.bookings.map((booking) => (
              ...
              >
                <div className="appointment-date">
                  <span>
                    {new Date(booking.appointmentAt).toLocaleDateString(
                      "zh-CN",
                      { weekday: "short" },
                    )}
                  </span>
                ...
                <div className="appointment-place">
                  ...
                  <div>
                    ...
                    <small>挂号号 {booking.id.slice(-6).toUpperCase()}</small>
                  </div>
                </div>
                {booking.status === "confirmed" ? (
          ...
          <p className="empty-inline">暂无预约记录</p>
        )}
      </div>
      <div className="service-section">
        <div className="section-heading">
          <div>
            <h2>可预约号源</h2>
            <p>请先选择要移交给医生的问诊报告，推荐科室号源会优先显示。</p>
          </div>
        </div>
        {availableReports.length ? (
          <label className="report-choice">
            ...
            <select
              ...
              onChange={(event) =>
                setSelectedConsultationId(event.target.value)
              }
            >
              {availableReports.map((item) => (
                <option value={item.consultationId} key={item.id}>
                  {new Date(item.createdAt).toLocaleString("zh-CN")} ·{" "}
        ...
        <div className="schedule-options">
          {sortedSchedules.map((schedule) => (
            <div className="panel schedule-option" key={schedule.id}>
              <div className="avatar large">
                {schedule.doctor.name.slice(0, 1)}
              </div>
              <div>
                <span>
                  {schedule.department} · {schedule.campus}
                </span>
                <h3>
                  {schedule.doctor.name} · {schedule.doctor.title}
                </h3>
                <p>
              ...
              <em>余 {schedule.remaining} 号</em>
              <button
                className="primary-button"
                disabled={bookingId === schedule.id || !availableReports.length}
                onClick={() => book(schedule)}
              >
                {bookingId === schedule.id ? "预约中…" : "确认预约"}
              </button>
            </div>
          ))}
        </div>
...
function LiveFollowups({ setActive }) {
  ...
  const [message, setMessage] = useState("");
  const load = () =>
    api.followups().then((result) => setItems(result.followups));
  useEffect(() => {
    load().catch((error) => setMessage(error.message));
  }, []);
  async function submit(id) {
    try {
      const result = await api.submitFollowup(id, form);
      setMessage(result.message);
      setActiveId("");
      localStorage.removeItem(`vertibalance_followup_draft_${id}`);
      await load();
    } catch (error) {
      setMessage(error.message);
    }
  ...
  function openTask(id) {
    setActiveId(id);
    const saved = localStorage.getItem(`vertibalance_followup_draft_${id}`);
    setForm(
      saved
        ? JSON.parse(saved)
        : { severity: 3, frequency: 0, text: "", medicationTaken: true },
    );
  }
  useEffect(() => {
    if (activeId)
      localStorage.setItem(
        `vertibalance_followup_draft_${activeId}`,
        JSON.stringify(form),
      );
  }, [activeId, form]);
    ...
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>康复随访</h1>
          <p>反馈真实症状变化；异常结果会标记给医生关注。</p>
        </div>
      </div>
      {message && (
        <div
          className={`inline-feedback ${message.includes("异常") || message.includes("就医") ? "warning" : "success"}`}
        >
          {message}
        </div>
      )}
          ...
          >
            ...
            <b>
              {key === "all"
                ? items.length
                : items.filter((item) =>
                    key === "abnormal" ? item.abnormal : item.status === key,
                  ).length}
            </b>
          </button>
        ))}
      ...
      {visible.length ? (
        <div className="followup-grid">
          {visible.map((item, index) => {
            ...
            return (
              <div
                className={`followup-card live ${item.abnormal ? "abnormal" : ""}`}
                key={item.id}
              >
                <div className={`task-icon t${index % 3}`}>
                ...
                <div>
                  ...
                  <p>
                    {item.status === "completed"
                      ? `已提交 · 严重程度 ${item.feedback?.severity}/10 · 过去 24 小时 ${item.feedback?.frequency ?? 0} 次`
                      : "请按计划记录当前症状、发作频率和用药情况"}
                  </p>
                </div>
                <em className={overdue ? "overdue" : ""}>
                  {item.abnormal
                    ? "异常关注"
                    : item.status === "completed"
                      ? "已完成"
                ...
                {item.status === "pending" && (
                  ...
                  >
                    {activeId === item.id ? "收起填写" : "开始填写"}
                  </button>
                )}
                {item.abnormal && item.status === "completed" && (
                  <button
                    className="outline-button"
                    onClick={() => setActive("reports")}
                  >
                ...
                {activeId === item.id && (
                  <div className="followup-form">
                    <label>
                      当前眩晕严重程度 <strong>{form.severity}/10</strong>
                      <input
                        type="range"
                        min="0"
                        max="10"
                    ...
                    <label>
                      ...
                      <textarea
                        ...
                        placeholder="例如：今天眩晕次数减少，但起床时仍明显…"
                      />
                    </label>
        ...
        <EmptyState
          icon={ClipboardCheck}
          title="暂无匹配随访"
          message="当前筛选下没有任务，切换到全部即可查看历史记录。"
        />
      )}
    </div>
  );
...
function knowledgeCategoryForReport(report) {
  ...
  if (direction.includes("耳石") || direction.includes("位置性"))
    return "耳石症";
...
function LiveKnowledge() {
  ...
  const recommendedCategory = items.some(
    (item) => item.category === inferredCategory,
  )
    ? inferredCategory
    : items.some((item) => item.category === "就医准备")
      ? "就医准备"
      : inferredCategory;
  const visibleItems =
    category || !recommendedCategory
      ...
      : [...items].sort(
          (a, b) =>
            Number(b.category === recommendedCategory) -
            Number(a.category === recommendedCategory),
        );
  return (
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>眩晕健康科普</h1>
          <p>科普内容与诊断结论分开呈现，仅用于健康教育。</p>
        </div>
      </div>
      {latestReport && (
        <div className="knowledge-reason">
          <Sparkles size={18} />
          <div>
            <strong>已结合最近一次问诊为你排序</strong>
            <span>
              {["emergency", "high"].includes(latestReport.riskLevel)
                ? "报告含较高风险提示，优先了解危险信号和及时就医。"
                : `报告方向与“${recommendedCategory}”相关，优先展示对应科普和就医准备。`}
            </span>
          </div>
          <button onClick={() => setCategory(recommendedCategory)}>
            只看相关推荐
          </button>
        </div>
      )}
      <div className="knowledge-filters">
        {categories.map((item) => (
          <button
            className={(category || "全部") === item ? "active" : ""}
            key={item}
      ...
      {error && <div className="inline-feedback">{error}</div>}
      {visibleItems.length ? (
        <div className="article-grid live">
          {visibleItems.map((item, index) => (
            <article
              className={
                !category && item.category === recommendedCategory
                  ? "recommended-article"
                  : ""
              }
              key={item.id}
            ...
            >
              <div className={`article-cover cover-${(index % 4) + 1}`}>
                <BookOpen size={30} />
              </div>
              <span className="tag">{item.category}</span>
              {!category && item.category === recommendedCategory && (
                <span className="report-match">报告相关</span>
              )}
              <h3>{item.title}</h3>
              <p>{item.summary}</p>
              <small>
        ...
        <EmptyState
          icon={BookOpen}
          title="暂无匹配科普"
          message="当前分类暂无文章，已建议你浏览全部眩晕健康知识。"
          action="查看全部科普"
        ...
        <div className="modal-backdrop" onClick={() => setSelected(null)}>
          ...
          >
            ...
            <div className="report-notice">
              <ShieldCheck size={17} />
              <div>
                <strong>健康教育提示</strong>
                <p>本文不构成诊断或治疗建议。如出现危险信号，请立即就医。</p>
              </div>
            </div>
          </article>
        </div>
...
function PatientDocuments() {
  ...
  const [category, setCategory] = useState("检查资料");
  const [consultationId, setConsultationId] = useState("");
  const [message, setMessage] = useState("");
  const [uploading, setUploading] = useState(false);
  const [preview, setPreviewState] = useState(null);
  const load = () =>
    Promise.all([api.uploads(), api.consultations()]).then(
      ([uploads, records]) => {
        setItems(uploads.uploads);
  ...
  async function upload() {
    if (!file) {
      setMessage("请先选择 PDF、JPG 或 PNG 文件");
      return;
    }
    setUploading(true);
    ...
    if (consultationId) body.append("consultationId", consultationId);
    try {
      await api.upload(body);
      setMessage("资料已加密保存，挂号移交后对应医生可查看。");
      setFile(null);
      await load();
    } catch (error) {
      setMessage(error.message);
  ...
  async function remove(item) {
    if (!window.confirm(`确认删除“${item.name}”？删除后无法恢复。`)) return;
    try {
  ...
  async function setPreview(item) {
    ...
    try {
      const response = await fetch(`/api/v1/uploads/${item.id}/preview`, {
        headers: { Authorization: `Bearer ${getAuthToken()}` },
      });
      if (!response.ok) throw new Error("资料预览失败或当前无访问权限");
      const blob = await response.blob();
      setPreviewState({ ...item, url: URL.createObjectURL(blob) });
    } catch (error) {
      setMessage(error.message);
    }
  }
  const previewUrl = (item) => item.url;
  async function download(item) {
    const response = await fetch(`/api/v1/uploads/${item.id}/download`, {
      headers: { Authorization: `Bearer ${getAuthToken()}` },
    });
    if (!response.ok) {
      setMessage("下载失败或当前无访问权限");
      return;
    }
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    ...
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>健康资料</h1>
          <p>上传检查单或病史图片，挂号后安全移交给对应医生。</p>
        </div>
      </div>
      {message && (
        <div
          className={`inline-feedback ${message.includes("已加密") || message.includes("已删除") ? "success" : ""}`}
        >
          {message}
        </div>
      )}
      <div className="document-layout">
        <div className="panel upload-box">
          ...
          <h2>上传病史资料</h2>
          <p>仅支持 PDF、JPG、PNG，单个文件不超过 5MB；禁止上传可执行文件。</p>
        ...
        <div className="panel document-list">
          ...
          {items.length ? (
            ...
            <p className="empty-inline">暂无上传资料</p>
          )}
        </div>
      </div>
      {preview && (
        <div className="modal-backdrop" onClick={() => setPreview(null)}>
          <section
            className="knowledge-modal document-preview"
            role="dialog"
            aria-modal="true"
          ...
          >
            ...
            {preview.mimeType === "application/pdf" ? (
              <iframe title={preview.name} src={previewUrl(preview)} />
            ) : (
              <img src={previewUrl(preview)} alt={preview.name} />
            )}
            <p className="preview-note">
              资料仅在当前授权会话中展示，医生需在挂号移交后访问。
            </p>
          </section>
        </div>
      )}
    ...
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>上午好，张医生</h1>
          <p>今日共有 8 位待接诊患者，其中 1 位需要优先关注。</p>
        </div>
        <div className="date-control">
          <CalendarDays size={17} />
          2026 年 8 月 24 日<ChevronDown size={15} />
        </div>
      </div>
            ...
            {patients.map((patient) => (
              ...
              >
                <span className="patient-cell">
                  <i className={`avatar ${patient.tone}`}>{patient.avatar}</i>
                  <span>
                ...
                <span className="symptom-cell">{patient.symptom}</span>
                <span className="muted">{patient.time}</span>
                <span>
                  <ChevronRight size={18} />
                </span>
              </button>
        ...
        <aside className="doctor-aside">
          <div className="panel priority-card">
            ...
            <div className="priority-patient">
              <div className="avatar rose">林</div>
              <div>
                <strong>林晓雯 · 56 岁</strong>
                <span>10 分钟前完成预问诊</span>
              </div>
            </div>
            <div className="risk-reason">
              <AlertTriangle size={18} />
              <p>
                <strong>发现危险信号</strong>
                <span>
                  突发眩晕并伴有无法独立行走，建议优先复核中枢性病变风险。
                </span>
              </p>
            </div>
            <button
              className="dark-button"
              onClick={() => onPatient(patients[0])}
            >
              立即查看资料
              <ArrowRight size={16} />
            </button>
          ...
          <div className="panel schedule-card">
            <div className="section-heading">
              ...
              <button className="text-button">全部</button>
            </div>
            {[
              ["09:30", "周建国", "初诊"],
              ["10:00", "陈雨桐", "复诊"],
              ...
              <div
                className={`schedule-row ${i === 0 ? "next" : ""}`}
                key={time}
              >
                <strong>{time}</strong>
                <i />
  ...
  return (
    <div className="page patient-detail">
      ...
      <div className="detail-head">
        <div className={`avatar xl ${patient.tone}`}>{patient.avatar}</div>
        <div>
          ...
          <p>
            {patient.gender} · {patient.age} 岁 · 患者编号 {patient.id}
          </p>
        </div>
      ...
      <div className="detail-grid">
        <section>
          <div className="panel clinical-summary">
            ...
            <div className="summary-highlight">
              <p>
                {patient.symptom}。症状于今晨突然出现，持续约 40
                分钟，伴恶心及明显行走不稳。否认耳鸣与听力下降。
              </p>
            </div>
            <div className="clinical-facts">
              ...
              <div>
                <span>持续时间</span>
                <strong>约 40 分钟</strong>
              </div>
              <div>
                <span>诱发因素</span>
                <strong>无明显体位诱发</strong>
              </div>
              <div>
                <span>既往史</span>
                <strong>高血压 8 年</strong>
              </div>
            </div>
            <div className="danger-box">
              <AlertTriangle size={20} />
              <div>
                <strong>规则引擎标记：无法独立行走</strong>
                <p>
                  危险信号优先级高于模型判断，建议优先完成神经系统查体并评估急诊处置。
                </p>
              </div>
            </div>
          </div>
          <div className="panel timeline-panel">
            ...
            <div className="clinical-timeline">
              <div>
                ...
                <span>08:10</span>
                <p>
                  <strong>眩晕突然发作</strong>
                  <small>起床后出现持续旋转感，无明确体位诱因</small>
                </p>
              </div>
              <div>
                <i />
                <span>08:25</span>
                <p>
                  <strong>出现行走不稳</strong>
                  <small>需扶墙行走，伴恶心，无呕吐</small>
                </p>
              </div>
              <div>
                <i />
                <span>09:04</span>
                <p>
                  <strong>完成 AI 预问诊</strong>
                  <small>规则筛查标记高风险并推荐优先就医</small>
                </p>
              </div>
            </div>
          </div>
        ...
        <aside>
          <div className="panel ai-assist">
            ...
            <button className="soft-button full">
              <Sparkles size={16} />
              重新生成分析
            </button>
            <p className="assist-note">
              AI 结果仅供参考，最终判断与处置由医生完成。
            </p>
          </div>
          <div className="panel record-form">
            ...
            <label>
              临床意见
              <textarea placeholder="记录诊断考虑、检查意见与处置建议…" />
            </label>
            ...
            <button
              ...
              onClick={() => window.alert("演示数据已在本地会话中保存")}
            >
              保存处置记录
            </button>
          </div>
...
function DoctorApp({ active }) {
  ...
  if (active === "workspace" || active === "patients")
    return <DoctorWorkspace onPatient={setSelected} />;
  return (
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>{active === "followups" ? "随访任务" : "我的排班"}</h1>
          <p>
            {active === "followups"
              ? "创建并跟踪患者诊后恢复计划"
              : "查看门诊时段与患者预约安排"}
        ...
        <button className="primary-button">
          <Plus size={17} />
          {active === "followups" ? "新建随访" : "申请调班"}
        </button>
      </div>
      {active === "followups" ? (
        <div className="followup-grid">
          {patients.slice(0, 4).map((p, i) => (
            <div className="followup-card" key={p.id}>
              <div className={`avatar ${p.tone}`}>{p.avatar}</div>
              <div>
                <span>{i < 2 ? "今天到期" : "本周计划"}</span>
                <h3>{p.name} · 症状恢复随访</h3>
                <p>复核眩晕频率、用药情况及康复训练完成度</p>
              </div>
              <em>{i === 0 ? "异常反馈" : "待完成"}</em>
              <button className="outline-button">查看任务</button>
            </div>
          ))}
        </div>
      ) : (
        <div className="calendar-placeholder panel">
          <CalendarDays size={44} />
          <h2>本周门诊排班</h2>
          <div className="week-strip">
            {["周一 24", "周二 25", "周三 26", "周四 27", "周五 28"].map(
              (d, i) => (
                <div className={i === 0 ? "active" : ""} key={d}>
                  <span>{d}</span>
                  <strong>{i === 2 ? "休诊" : "08:30–12:00"}</strong>
                  <small>{i === 2 ? "—" : "眩晕专病门诊"}</small>
                </div>
              ),
            )}
          </div>
    ...
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>我的排班</h1>
          <p>由管理员统一配置，号源余量随患者挂号实时更新。</p>
        </div>
      </div>
      <div className="schedule-options">
        {schedules.map((item) => (
          <div className="panel schedule-option" key={item.id}>
            <div className="task-icon">
              <CalendarDays size={20} />
            </div>
            <div>
              <span>
                {item.department} · {item.campus}
              </span>
              <h3>{new Date(item.startAt).toLocaleString("zh-CN")}</h3>
              <p>
                至{" "}
            ...
            <em>
              {item.remaining}/{item.capacity} 号
            </em>
    ...
    <div className="page">
      <div className="page-title">
        <div>
          ...
          <h1>运营总览</h1>
          <p>诊疗服务、模型调用和安全态势一览。</p>
        </div>
        <div className="date-control">
          <CalendarDays size={17} />
          最近 7 天<ChevronDown size={15} />
        </div>
      </div>
        ...
        <section className="panel model-health">
          ...
          <div className="model-name">
            ...
            <p>
              ...
              <span>生产版本 · Prompt v1.3</span>
            </p>
            <button className="icon-button" onClick={() => setActive("models")}>
              <ChevronRight size={18} />
            </button>
          </div>
...
function AdminSection({ active }) {
  const config = {
    users: ["用户与角色", "管理平台账号、医生资质与访问权限", UserRoundCog],
    schedules: ["医生与排班", "配置科室、出诊时段与可用号源", CalendarDays],
    models: ["模型管理", "管理模型版本、提示词与调用状态", Brain],
    knowledge: ["知识库", "维护医疗科普、问诊模板和危险信号规则", Database],
    audit: ["审计与安全", "追踪关键操作、访问记录与安全告警", ShieldCheck],
  };
      ...
      <div className="page">
        ...
        <div className="panel data-panel">
          ...
          <div className="simple-table">
            ...
            {auditRows.map((row) => (
              <div className="simple-row" key={row.time}>
                <span>{row.time}</span>
                <span>{row.actor}</span>
                <strong>{row.action}</strong>
                <span>{row.object}</span>
                <em className={row.status === "已拦截" ? "blocked" : "ok"}>
                  {row.status}
                </em>
              </div>
            ))}
    ...
    return (
      <div className="page">
        ...
        <div className="model-page-grid">
          <div className="panel model-config">
            <div className="config-head">
              ...
              <div>
                ...
                <p>通过 OpenAI 兼容协议接入 · Prompt v1.3</p>
              </div>
              <button className="outline-button">
                <Settings size={16} />
                配置
            ...
            <div className="endpoint-box">
              <span>服务地址</span>
              <code>https://api.modagent-homing.com/v1</code>
              <ShieldCheck size={17} />
              <small>API 密钥仅存于服务端环境变量</small>
            </div>
          ...
          <div className="panel">
            ...
            <div className="coming-soon">
              ...
              <strong>灰度发布接口已预留</strong>
              <p>
                正式数据库与任务队列接入后，可支持新会话灰度切换及自动回滚。
              </p>
              <span>下一阶段</span>
            </div>
          </div>
  ...
  return (
    <div className="page">
      ...
      <div className="panel placeholder-panel">
        ...
        <h2>{title}基础界面已就绪</h2>
        <p>
          本阶段已完成页面框架与交互入口。真实增删改查、权限验证和数据库事务将在后续服务端迭代接入。
        </p>
...
function LiveAdminDashboard({ setActive }) {
  ...
  return (
    <div className="page">
      ...
      <div className="stats-grid admin-metrics">
        <StatCard
          icon={MessageCircleMore}
          tone="blue"
          label="累计问诊"
          value={m.consultations}
          detail={`平台用户 ${m.users} 人`}
        />
        <StatCard
          icon={CalendarDays}
          tone="violet"
          label="挂号转化率"
          value={`${m.bookingConversion}%`}
          detail="已结束问诊转化"
        />
        <StatCard
          icon={AlertTriangle}
        ...
        <StatCard
          ...
          tone="green"
          label="平均问诊时长"
          value={`${m.averageConsultationMinutes}m`}
          detail="已结束会话"
        />
        <StatCard
          icon={Stethoscope}
      ...
      <div className="admin-grid">
        <section className="panel model-health">
          ...
          <div className="model-name">
            ...
            <p>
              <strong>{model.config?.model || "未配置"}</strong>
              <span>
                {model.config?.promptVersion} · {model.config?.status}
              </span>
            </p>
            <button className="icon-button" onClick={() => setActive("models")}>
              <ChevronRight size={18} />
          ...
          <div className="model-metrics">
            <div>
              <span>调用成功率</span>
              <strong>{m.modelSuccessRate}%</strong>
              <i>
                <b style={{ width: `${m.modelSuccessRate}%` }} />
              </i>
            </div>
            <div>
              <span>平均响应</span>
            ...
            <div>
              <span>今日调用</span>
              <strong>{model.callsToday}</strong>
              <small>失败 {model.failures} 次</small>
            </div>
          </div>
        </section>
        <section className="panel governance-card">
          <div className="section-heading">
            <div>
              <h2>平台治理入口</h2>
              <p>关键配置均记录审计</p>
...
function LiveAdminUsers() {
  ...
  const load = () => api.adminUsers().then((result) => setUsers(result.users));
  useEffect(() => {
    load();
  }, []);
  ...
  return (
    <div className="page">
      ...
      {message && <div className="inline-feedback success">{message}</div>}
      {show && (
        <div className="panel create-form user-form">
          <label>
            角色
            <select
              value={form.role}
      ...
      <div className="panel data-panel">
        <div className="simple-table">
          <div className="simple-head user-table">
            <span>用户</span>
            <span>角色</span>
            <span>账号</span>
            <span>科室/资质</span>
          ...
          {users.map((user) => (
            <div className="simple-row user-table" key={user.id}>
              <span className="patient-cell">
                <i className="avatar blue">{user.name.slice(0, 1)}</i>
                <strong>{user.name}</strong>
              </span>
              <span>{user.role}</span>
              <span>{user.account}</span>
              ...
              <em className={user.status === "active" ? "ok" : "blocked"}>
                {user.status}
              </em>
              <button
                className="outline-button"
                onClick={() => toggle(user)}
                disabled={user.account === "admin@demo.com"}
              >
...
function LiveAdminSchedules() {
  ...
  const load = () =>
    Promise.all([
      ...
      setDoctors(
        b.users.filter((u) => u.role === "doctor" && u.status === "active"),
      );
      ...
      setForm((current) => ({
        ...
        doctorId:
          current.doctorId ||
          b.users.find((u) => u.role === "doctor" && u.status === "active")
            ?.id ||
          "",
        departmentId: current.departmentId || c.departments[0]?.id || "",
      }));
    });
  useEffect(() => {
    load();
  ...
  async function create() {
    try {
      await api.createSchedule({ ...form, capacity: Number(form.capacity) });
      setShow(false);
      setMessage("排班已创建并立即进入患者端号源列表。");
      await load();
    } catch (error) {
      setMessage(error.message);
    }
  ...
  return (
    <div className="page">
      ...
      {message && <div className="inline-feedback success">{message}</div>}
      {show && (
        <div className="panel create-form schedule-form">
          <label>
            医生
            <select
              value={form.doctorId}
      ...
      <div className="schedule-options admin-schedules">
        {schedules.map((item) => (
          <div className="panel schedule-option" key={item.id}>
            <div className="avatar large">
              {item.doctor?.name?.slice(0, 1) || "医"}
            </div>
            <div>
              <span>
                {item.department} · {item.campus}
              ...
              <p>
                号源 {item.remaining}/{item.capacity}
              </p>
            </div>
            <em>{item.status}</em>
            <button className="outline-button" onClick={() => close(item)}>
              {item.status === "open" ? "关闭号源" : "重新开放"}
            </button>
          </div>
        ))}
      </div>
...
function LiveAdminKnowledge() {
  ...
  const [ruleForm, setRuleForm] = useState({ label: "", keywords: "" });
  const load = () =>
    Promise.all([api.adminKnowledge(), api.riskRules()]).then(([a, b]) => {
      setItems(a.items);
      setRules(b.rules);
    });
  useEffect(() => {
  ...
  async function toggle(item) {
    await api.updateKnowledge(item.id, {
  ...
  async function createRule() {
    try {
      ...
      setRuleForm({ label: "", keywords: "" });
      setMessage("危险信号规则已新增并立即生效。");
      await load();
    } catch (error) {
      setMessage(error.message);
    }
  ...
  async function toggleRule(rule) {
    await api.updateRiskRule(rule.id, { enabled: !rule.enabled });
    await load();
  }
  return (
    <div className="page">
      ...
      <div className="knowledge-admin-list">
        {items.map((item) => (
          <div className="panel" key={item.id}>
            ...
            <em className={item.status === "published" ? "ok" : "blocked"}>
              {item.status}
            </em>
            <button className="outline-button" onClick={() => toggle(item)}>
      ...
      <section className="risk-rule-section">
        <div className="section-heading">
          <div>
            <h2>危险信号规则</h2>
            <p>关键词以逗号分隔；启用后对下一条患者消息立即生效</p>
          </div>
        </div>
        <div className="panel create-form risk-form">
          <label>
            信号名称
            <input
          ...
          <label>
            ...
            <input
              ...
              placeholder="吞咽困难，喝水呛咳"
            />
          </label>
          <button className="dark-button" onClick={createRule}>
            新增规则
          </button>
        </div>
        <div className="risk-rule-list">
          {rules.map((rule) => (
            <div className="panel" key={rule.id}>
              ...
              <em className={rule.enabled ? "ok" : "blocked"}>
...
function LiveAdminModels() {
  ...
  const [form, setForm] = useState({
    model: "",
    baseUrl: "https://api.modagent-homing.com/v1",
    promptVersion: "v1.4",
  });
  const load = () => api.adminModels().then(setData);
  useEffect(() => {
    load();
  }, []);
  if (!data) return <DataLoading label="正在读取模型配置与脱敏调用日志…" />;
  const config =
    data.configs.find((item) => item.status === "active") || data.configs[0];
  const success = data.calls.length
    ? Math.round(
        (data.calls.filter((item) => item.success).length / data.calls.length) *
          100,
      )
    : 100;
  async function create() {
    try {
      ...
      setShow(false);
      setMessage("模型配置已创建；激活前会检查服务可达性。");
      await load();
    } catch (error) {
      setMessage(error.message);
    }
  ...
  async function activate(item) {
    ...
    try {
      await api.activateModel(item.id);
      setMessage("模型已切换。新问诊使用新版本，进行中的会话保持原版本。");
      await load();
    } catch (error) {
      setMessage(error.message);
    } finally {
  ...
  return (
    <div className="page">
      ...
      {message && <div className="inline-feedback success">{message}</div>}
      {show && (
        <div className="panel create-form model-create">
          <label>
            模型名
            <input
              value={form.model}
      ...
      <div className="model-page-grid">
          ...
          <div className="panel model-config">
            <div className="config-head">
              ...
              <div>
                ...
                <h2>{config?.model}</h2>
                <p>{config?.promptVersion} · OpenAI 兼容协议</p>
              </div>
            </div>
            <div className="endpoint-box">
              <span>服务地址</span>
            ...
            <div className="model-stats-row">
              ...
              <div>
                <span>调用记录</span>
                <strong>{data.calls.length}</strong>
              </div>
              <div>
                <span>成功</span>
                <strong>{data.calls.filter((c) => c.success).length}</strong>
              </div>
              <div>
                <span>失败</span>
                <strong>{data.calls.filter((c) => !c.success).length}</strong>
            ...
            {data.configs.map((item) => (
              <div className="panel" key={item.id}>
                <div>
                  ...
                  <span>
                    {item.promptVersion} · {item.baseUrl}
                  </span>
                </div>
                <em className={item.status === "active" ? "ok" : "blocked"}>
                  {item.status}
                </em>
                {item.status !== "active" && (
                  <button
        ...
        <div className="panel model-call-list">
          <h2>最近调用</h2>
          {data.calls.length ? (
            data.calls.slice(0, 8).map((call) => (
              <div key={call.id}>
                <span>{new Date(call.createdAt).toLocaleString("zh-CN")}</span>
                <strong>{call.latencyMs}ms</strong>
                <em className={call.success ? "ok" : "blocked"}>
                  {call.success ? "成功" : call.error}
                </em>
              </div>
            ))
          ) : (
            <p className="empty-inline">暂无模型调用记录</p>
          )}
        </div>
      </div>
    </div>
...
function LiveAdminAudit() {
  const [items, setItems] = useState([]);
  useEffect(() => {
    api.adminAudits().then((result) => setItems(result.audits));
  }, []);
  return (
    <div className="page">
      <div className="page-title">
        ...
        <div className="simple-table">
          ...
          {items.map((item) => (
            <div className="simple-row" key={item.id}>
              ...
              <span>
                {item.objectType} · {item.objectId.slice(-10)}
              </span>
              <em className={item.status === "success" ? "ok" : "blocked"}>
                {item.status}
              </em>
            </div>
          ))}
...
function LiveAdminApp({ active, setActive }) {
  if (active === "dashboard")
    return <LiveAdminDashboard setActive={setActive} />;
...
function App() {
    ...
    if (!getAuthToken()) setSession(null);
      ...
      api
        ...
        .then((result) => {
          ...
          setActive(defaults[result.user.role]);
        })
        .catch(logout);
  ...
  const content = (() => {
    if (role === "patient") {
      if (active === "overview")
        return (
          <LivePatientOverview setActive={setActive} user={session.user} />
        );
      if (active === "consult")
        return (
          <Consultation
      ...
      if (active === "appointments")
        return <LiveAppointments setActive={setActive} />;
      if (active === "followup") return <LiveFollowups setActive={setActive} />;
  ...
  return (
    <Shell
      ...
      user={session.user}
      onLogout={logout}
      onUserUpdate={(user) => setSession((current) => ({ ...current, user }))}
      active={active}
      setActive={setActive}
    >
      {content}
