...
export function screenRisk(content = '', customRules = []) {
  ...
  return {
    dangerSignals,
    riskLevel: dangerSignals.length ? 'high' : highRiskHistory ? 'high' : 'medium',
  };
}
...
export async function callMedicalModel(messages, systemPrompt = MEDICAL_GUARDRAIL, pinnedConfig = null) {
  ...
  const timeoutMs = Number(process.env.MEDCHAT_TIMEOUT_MS || 30000);
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({
        model, temperature: 0.25, max_tokens: Number(process.env.MEDCHAT_MAX_TOKENS || 1200),
    ...
    if (error.name === 'AbortError') {
      const timeoutError = new Error('MODEL_TIMEOUT');
...
export function buildReport(consultation, messages) {
  ...
  const riskLevel = consultation.riskLevel === 'emergency' ? 'high' : consultation.riskLevel || 'medium';
  return {
